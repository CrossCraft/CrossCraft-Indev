...
{
	auto m = RenderContext::get().get_projection_matrix() *
		 RenderContext::get().get_view_matrix();
	m = m.Transpose();
	...
	mathfu::Vector<float, 3> crosses[Combinations] = {
		mathfu::CrossProductHelper((m_planes[Left]).xyz(),
					   (m_planes[Right]).xyz()),
		mathfu::CrossProductHelper((m_planes[Left]).xyz(),
					   (m_planes[Bottom]).xyz()),
		mathfu::CrossProductHelper((m_planes[Left]).xyz(),
		...
		mathfu::CrossProductHelper((m_planes[Left]).xyz(),
					   (m_planes[Far]).xyz()),
		mathfu::CrossProductHelper((m_planes[Right]).xyz(),
					   (m_planes[Bottom]).xyz()),
		mathfu::CrossProductHelper((m_planes[Right]).xyz(),
					   (m_planes[Top]).xyz()),
		mathfu::CrossProductHelper((m_planes[Right]).xyz(),
		...
		mathfu::CrossProductHelper((m_planes[Right]).xyz(),
					   (m_planes[Far]).xyz()),
		mathfu::CrossProductHelper((m_planes[Bottom]).xyz(),
					   (m_planes[Top]).xyz()),
		mathfu::CrossProductHelper((m_planes[Bottom]).xyz(),
					   (m_planes[Near]).xyz()),
		mathfu::CrossProductHelper((m_planes[Bottom]).xyz(),
					   (m_planes[Far]).xyz()),
		mathfu::CrossProductHelper((m_planes[Top]).xyz(),
					   (m_planes[Near]).xyz()),
		mathfu::CrossProductHelper((m_planes[Top]).xyz(),
					   (m_planes[Far]).xyz()),
		mathfu::CrossProductHelper((m_planes[Near]).xyz(),
					   (m_planes[Far]).xyz())
	};
...
bool Frustum::IsBoxVisible(const mathfu::Vector<float, 3> &minp,
			   const mathfu::Vector<float, 3> &maxp) const
{
	...
	for (int i = 0; i < Count; i++) {
		...
		for (const auto &corner : boxCorners) {
			if (mathfu::DotProductHelper(m_planes[i], corner) >=
			    0.0f) {
				isInside = true;
				break;
			}
...
Frustum::intersection(const mathfu::Vector<float, 3> *crosses) const
{
	float D = mathfu::DotProductHelper(m_planes[a].xyz(),
					   crosses[ij2k<b, c>::k]);
	...
	mathfu::Vector<float, 3> res =
		mat * mathfu::Vector<float, 3>(m_planes[a].w, m_planes[b].w,
					       m_planes[c].w);
	return res * (-1.0f / D);
}