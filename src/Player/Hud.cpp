...
HUD::HUD()
{
	...
	water_sprite = create_scopeptr<Graphics::G2D::Sprite>(
		ResourcePack::get().get_texture("water_overlay"),
		Rendering::Rectangle{ { 0.0f, 0.0f }, { 480.0f, 272.0f } });
	water_sprite->set_layer(-5);

	crosshair = create_scopeptr<Graphics::G2D::Sprite>(
		ResourcePack::get().get_texture("gui_common"),
		Rendering::Rectangle{ { 232.0f, 128.0f }, { 16.0f, 16.0f } },
	...
	heartHalf = create_scopeptr<Graphics::G2D::Sprite>(
		icons_texture, Rendering::Rectangle{ { 147, 27 }, { 7, 7 } },
...
auto HUD::draw_hearts(Player *player) -> void
{
	...
	for (int i = 0; i < 10; i++) {
		...
		if (player->health <= 5) {
			Rendering::RenderContext::get().matrix_translate(
				{ (float)off, sinf(i + heartTimer * 25),
				  0.0f });
		} else {
			Rendering::RenderContext::get().matrix_translate(
				{ (float)off, 0.0f, 0.0f });
...
void HUD::draw(Player *player, double dt)
{
	...
	Rendering::RenderContext::get().matrix_clear();
	auto ipos = mathfu::Vector<int, 3>(
		player->position.x, player->position.y, player->position.z);

	font_render->clear();
	font_render->draw_text_aligned(CC_TEXT_COLOR_YELLOW, CC_TEXT_ALIGN_LEFT,
				       "POS: " + std::to_string(ipos.x) + " " +
					       std::to_string(ipos.y) + " " +
					       std::to_string(ipos.z),
				       mathfu::Vector<float, 2>(0, 264), 3.0f);
	font_render->draw_text_aligned(CC_TEXT_COLOR_YELLOW,
				       CC_TEXT_ALIGN_RIGHT,
				       "FPS: " + std::to_string(currentFPS),
				       mathfu::Vector<float, 2>(480, 264),
				       3.0f);
