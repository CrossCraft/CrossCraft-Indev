...
InGameMenu::InGameMenu()
{
	...
	button_disabled = create_refptr<Graphics::G2D::Sprite>(
		...
		Rendering::Rectangle{ { -100, -10 }, { 200, 20 } },
		Rendering::Rectangle{ { 0.0f, 46.0f / 256.0f },
	...
	button_unselected = create_refptr<Graphics::G2D::Sprite>(
		ResourcePack::get().get_texture("gui_common"),
		Rendering::Rectangle{ { -100, -10 }, { 200, 20 } },
		Rendering::Rectangle{ { 0.0f, 66.0f / 256.0f },
				      { 200.0f / 256.0f, 20.0f / 256.0f } });
	...
	button_selected = create_refptr<Graphics::G2D::Sprite>(
		ResourcePack::get().get_texture("gui_common"),
		Rendering::Rectangle{ { -100, -10 }, { 200, 20 } },
		Rendering::Rectangle{ { 0.0f, 86.0f / 256.0f },
				      { 200.0f / 256.0f, 20.0f / 256.0f } });
...
auto InGameMenu::left_click_command(std::any p) -> void
{
	...
	if (get().is_dead) {
		if (mouseVec.x >= 190 && mouseVec.x <= 290 &&
		    mouseVec.y >= 76 && mouseVec.y <= 96) {
			auto packet = CC_EventPacket_Create_PlayerRespawn();
			CC_EventLoop_PushPacketOutbound(
				ELoop::get().client_event_loop, &packet);
		...
		if (mouseVec.x >= 190 && mouseVec.x <= 290 &&
		    mouseVec.y >= 130 && mouseVec.y <= 150) {
			get().toggle();
		} else if (mouseVec.x >= 190 && mouseVec.x <= 290 &&
			   mouseVec.y >= 106 && mouseVec.y <= 126) {
			SC_APP_INFO("Unimplemented");
		} else if (mouseVec.x >= 190 && mouseVec.x <= 290 &&
			   mouseVec.y >= 82 && mouseVec.y <= 102) {
...
auto InGameMenu::draw_button(const mathfu::Vector<float, 2> &mousePos,
			     mathfu::Vector<float, 2> pos,
			     const std::string &text, bool disabled) -> void
{
	mathfu::Vector<float, 2> windowSize(480, 272);
	auto mouseVec = mousePos * windowSize;
	mouseVec.y = windowSize.y - mouseVec.y;
	...
	Rendering::RenderContext::get().matrix_translate(
		mathfu::Vector<float, 3>(pos.x, pos.y, 6));

	if (disabled) {
		button_disabled->draw();
		pos.y -= 4;
		font_render->draw_text_aligned(CC_TEXT_COLOR_GRAY,
					       CC_TEXT_ALIGN_CENTER, text, pos,
					       20.0f);
	} else {
		if (mouseVec.x >= pos.x - 100 && mouseVec.x <= pos.x + 100 &&
		    mouseVec.y >= pos.y - 10 && mouseVec.y <= pos.y + 10) {
			button_selected->draw();
			pos.y -= 4;
			font_render->draw_text_aligned(CC_TEXT_COLOR_SELECT,
						       CC_TEXT_ALIGN_CENTER,
						       text, pos, 20.0f);
		} else {
			button_unselected->draw();
			pos.y -= 4;
			font_render->draw_text_aligned(CC_TEXT_COLOR_WHITE,
						       CC_TEXT_ALIGN_CENTER,
						       text, pos, 20.0f);
		}
	}
...
void InGameMenu::draw()
{
	...
	if (is_dead) {
		...
		font_render->clear();
		font_render->draw_text_aligned(
			CC_TEXT_COLOR_WHITE, CC_TEXT_ALIGN_CENTER, "Game over!",
			mathfu::Vector<float, 2>(240, 192 - 24), 20.0f);
		font_render->draw_text_aligned(
			CC_TEXT_COLOR_WHITE, CC_TEXT_ALIGN_CENTER, "Score: 0",
			mathfu::Vector<float, 2>(240, 168 - 24), 20.0f);

		draw_button(mouseVec, mathfu::Vector<float, 2>(240, 120 - 24),
			    "Respawn", false);
		draw_button(mouseVec, mathfu::Vector<float, 2>(240, 96 - 24),
			    "Title menu", false);
	...
	font_render->clear();
	font_render->draw_text_aligned(CC_TEXT_COLOR_WHITE,
				       CC_TEXT_ALIGN_CENTER, "Game Menu",
				       mathfu::Vector<float, 2>(240, 216),
				       20.0f);

	draw_button(mouseVec, mathfu::Vector<float, 2>(240, 140),
		    "Back to Game", false);
	draw_button(mouseVec, mathfu::Vector<float, 2>(240, 116), "Options",
		    true);
	draw_button(mouseVec, mathfu::Vector<float, 2>(240, 92), "Save Game",
		    false);
