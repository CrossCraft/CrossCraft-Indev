...
auto ResourcePack::get_path_index(std::string path) -> int
{
	...
	for (long int idx = packQueue.size() - 1; idx >= 0; idx--) {
		if (unzLocateFile(packs[packQueue[idx]], path.c_str(), 0) ==
		    UNZ_OK) {
			path_index = idx;
			break;
		}
...
auto ResourcePack::read_file_to_buffer(std::string packname, size_t &length)
	-> uint8_t *
{
	unzOpenCurrentFile(packs[packname]);

	unz_file_info file_info;
	unzGetCurrentFileInfo(packs[packname], &file_info, nullptr, 0, nullptr,
			      0, nullptr, 0);
...
auto ResourcePack::load_resource(std::string name, ResourceType type,
				 const std::string &path, bool needPix) -> void
{
	if (type == ResourceType::TEXTURE) {
		...
		auto result = Rendering::TextureManager::get().load_texture_ram(
			dataBuffer, arrayLength, SC_TEX_FILTER_NEAREST,
...
{
	load_resource("default", ResourceType::TEXTURE,
		      "assets/minecraft/textures/default.png", true);
	load_resource("terrain", ResourceType::TEXTURE,
		      "assets/minecraft/textures/terrain.png");
	load_resource("water_overlay", ResourceType::TEXTURE,
		      "assets/minecraft/textures/water.png");
	load_resource("gui_common", ResourceType::TEXTURE,
		      "assets/minecraft/textures/gui/gui.png");
	load_resource("items", ResourceType::TEXTURE,
		      "assets/minecraft/textures/gui/items.png");
	load_resource("icons", ResourceType::TEXTURE,
		      "assets/minecraft/textures/gui/icons.png");
	load_resource("death_overlay", ResourceType::TEXTURE,
		      "assets/crosscraft/textures/overlay.png");
	load_resource("inventory_overlay", ResourceType::TEXTURE,
		      "assets/minecraft/textures/gui/inventory.png");
	load_resource("clouds", ResourceType::TEXTURE,
