...
Sky::Sky() : last_tick(0), scroll(0.0)
{
	mesh = create_refptr<Rendering::Mesh<Rendering::Vertex> >();
	mesh2 = create_refptr<Rendering::Mesh<Rendering::Vertex> >();
	clouds = create_refptr<Rendering::Mesh<Rendering::Vertex> >();
	generate(Rendering::Color{ { 255, 255, 255, 255 } },
		 Rendering::Color{ { 255, 255, 255, 255 } });

	cloud_id = ResourcePack::get().get_texture("clouds");
}

void Sky::generate(Rendering::Color color, Rendering::Color color2)
{
	generateMesh(color, mesh);
	generateMesh(color2, mesh2);
}
void Sky::generateMesh(const Rendering::Color &color,
		       RefPtr<Rendering::Mesh<Rendering::Vertex>> mesh, bool useScroll)
{
	mesh->delete_data();
	mesh->vertices.clear();
	mesh->indices.clear();
	...
	for (int x = 0; x < 16; x++) {
		for (int z = 0; z < 16; z++) {
			...
			mesh->vertices.push_back(Rendering::Vertex{
				uvs[0] * (1 / 128.f) + ((float)x / 128.f + vOff),
			...
			mesh->vertices.push_back(Rendering::Vertex{
				uvs[2] * (1 / 128.f) + ((float)x / 128.f + vOff),
			...
			mesh->vertices.push_back(Rendering::Vertex{
				uvs[4] * (1 / 128.f) + ((float)x / 128.f + vOff),
			...
			mesh->vertices.push_back(Rendering::Vertex{
				uvs[6] * (1 / 128.f) + ((float)x / 128.f + vOff),
...
Rendering::Color HSL2RGB(float h, float sl, float l)
{
	...
	return Rendering::Color{ { static_cast<u8>(r * 255.0f),
...
auto blendColor(Rendering::Color source, Rendering::Color destination)
{
	return Rendering::Color{
		{ static_cast<u8>(blend((float)source.rgba.r / 255.0f,
					(float)destination.rgba.r / 255.0f) *
				  255.0f),
		  static_cast<u8>(blend((float)source.rgba.g / 255.0f,
					(float)destination.rgba.g / 255.0f) *
				  255.0f),
		  static_cast<u8>(blend((float)source.rgba.b / 255.0f,
					(float)destination.rgba.b / 255.0f) *
				  255.0f),
		  static_cast<u8>(blend((float)source.rgba.a / 255.0f,
					(float)destination.rgba.a / 255.0f) *
				  255.0f) }
	};
}

void Sky::draw(double dt, mathfu::Vector<float, 3> cam_pos, uint64_t tick)
{
	scroll += dt;
	...
	if (x > 1)
		x = 1;
	auto CelestialAngle =
		x + ((1.0 - ((float)cos(x * 3.14159) + 1.0) / 2.0) - x) / 3.0;
	...
	auto BaseColor = HSL2RGB(0.6222222f - temp * 0.05f, 0.65f + temp * 0.1f,
				 BrightnessModifier * 0.707f);
	...
	auto WorldFogColor =
		Rendering::Color{ { static_cast<u8>(r), static_cast<u8>(g),
	...
	if (tick != last_tick) {
		...
		auto colVec =
			mathfu::Vector<float, 3>(WorldSkyColor.rgba.r / 255.0f,
						 WorldSkyColor.rgba.g / 255.0f,
						 WorldSkyColor.rgba.b / 255.0f);
		colVec *= mathfu::Vector<float, 3>(0.2f, 0.2f, 0.6f);
		colVec += mathfu::Vector<float, 3>(0.04f, 0.04f, 0.1f);
		auto VoidColor =
			Rendering::Color{ { static_cast<u8>(colVec.x * 255.0f),
					    static_cast<u8>(colVec.y * 255.0f),
					    static_cast<u8>(colVec.z * 255.0f),
					    255 } };
		generate(WorldSkyColor, VoidColor);
	}

	generateMesh(Rendering::Color{ { WorldFogColor.rgba.r, WorldFogColor.rgba.g, WorldFogColor.rgba.b, 168 } }, clouds, true);
	...
	Rendering::RenderContext::get().matrix_translate(
		mathfu::Vector<float, 3>(-128.0f, realY, -128.0f) + cam_pos);
	Rendering::RenderContext::get().matrix_scale({ 1.0f, 1.0f, 1.0f });
#else
	Rendering::RenderContext::get().matrix_translate(
		mathfu::Vector<float, 3>(-256.0f, realY, -256.0f) + cam_pos);
	...
	Rendering::RenderContext::get().matrix_clear();
	Rendering::RenderContext::get().matrix_translate(
		mathfu::Vector<float, 3>(-256.0f, realY2, -256.0f) + cam_pos);
	Rendering::RenderContext::get().matrix_scale({ 2.0f, 1.0f, 2.0f });
	GI::set_culling_mode(true, false);
	mesh2->draw();
	GI::set_culling_mode(true, true);

	auto realY3 = 64.0f - cam_pos.y;

	GI::enable(GI_TEXTURE_2D);
	Rendering::TextureManager::get().bind_texture(cloud_id);

	Rendering::RenderContext::get().matrix_clear();
	Rendering::RenderContext::get().matrix_translate(
		mathfu::Vector<float, 3>(-128.0f, realY3, -128.0f) + cam_pos);
//	Rendering::RenderContext::get().matrix_scale({ 2.0f, 1.0f, 2.0f });
	clouds->draw();
}
}